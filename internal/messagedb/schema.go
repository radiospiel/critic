...
// schema_v4 adds _db_version table and triggers for change detection
var schema_v4 = `
	-- Settings table for metadata
	CREATE TABLE IF NOT EXISTS settings (
		key TEXT PRIMARY KEY,
...
func (db *DB) createInitialSchema() error {
	// Create tables using schema v4 (includes _db_version table and triggers)
	_, err := db.db.Exec(schema_v4)
	if err != nil {
